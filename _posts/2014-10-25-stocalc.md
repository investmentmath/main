...
* The variation of order $p$ of a process $M$ over an interval $[0,T]$ is the sup of: $$V^{(p)}(\pi_{[0,T]})(\omega)=\sum_{k=1}^{n}|M_{t_{k}}(\omega)-M_{t_{k-1}}(\omega)|^{p},$$ over all partitions $\pi_{[0,T]}=\{t_{0}=0,t_{1},t_{2},\ldots,t_{n}=T\}$.

* Finite (first order) variation processes have $\sup_{\pi_{[0,T]}} V^{(1)}(\pi_{[0,T]})(\omega)<\infty$ a.e.. They are amenable to Lebesgue Stieljes integration trajectory by trajectory. Variations of higher order are equal to $0$.

## Quadratic variation (2) 

* For the Brownian motion, we have $\sup_{\pi_{[0,T]}} V^{(1)}(\pi_{[0,T]})(\omega)=\infty$ and $\sup_{\pi_{[0,T]}} V^{(2)}(\pi_{[0,T]})(\omega)=T$ a.e.. Variations of higher order are zero.
...
$$[M,N]_{t}=\sup_{\pi_{[0,T]}} \sum_{k=1}^{n}(M_{t_{k}}(\omega)-M_{t_{k-1}}(\omega))(N_{t_{k}}(\omega)-N_{t_{k-1}}(\omega)).$$
